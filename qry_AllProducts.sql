SELECT quni_AllProducts_step_1.*, Switch(
	[Source_Code] IN ('TSO','OSB','OSN','PXC','PXD','SRN','SRS','TCS','E2J'), 'Tele',
	TRUE,'OSS') AS Channel, IIf([TL_Code] in (SELECT DISTINCT [TL_Code] FROM [Telesales Office]), 1, NULL) AS TS_Office, Switch(
	Channel = 'OSS' And [TS_Office] = 1,'OSS_Tele',
	Channel = 'OSS','Direct',
	Channel = 'Tele','Telesales') AS Channel_Sub, Switch(
